...
def DeriveRolePermissionPolicy(contractAddress, contractName, file):
    try:
        reads, reads2, writes = getRWofContract(file, contractAddress, contractName)
    except:
        traceback.print_exc()
    ...
    return reads, reads2, writes, createInformationSecurityPolicy

def runRoleMiningForSingleContract(address, contractName, simratio, workdir):
        ...
        reads, reads2, writes, createInformationSecurityPolicy = DeriveRolePermissionPolicy(contractAddress=address, contractName=contractName, file=os.path.join(workdir, address, contractName+".sol"))
        ...
        return observed_methods, func_mappings, reads, reads2, writes, security_policy, [] 